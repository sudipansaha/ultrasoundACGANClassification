...
#command to run:
##     python main.py --dataset folder --dataroot ./dataset/set1Training/ --outf ./checkpoints/set1/ --niter 200 --cuda
## or: python main.py --dataset folder --dataroot ./dataset/set2Training/ --outf ./checkpoints/set2/ --niter 200 --cuda
## or: python main.py --dataset folder --dataroot ./dataset/set3Training/ --outf ./checkpoints/set3/ --niter 200 --cuda
## or: python main.py --dataset folder --dataroot ./dataset/set4Training/ --outf ./checkpoints/set4/ --niter 200 --cuda
## or: python main.py --dataset folder --dataroot ./dataset/set5Training/ --outf ./checkpoints/set5/ --niter 200 --cuda
...
###Much of the code in this part is hard coded. So be careful
if 'set1' in opt.dataroot:
    testDataRoot = './dataset/set1Test/'
elif 'set2' in opt.dataroot:
    testDataRoot = './dataset/set2Test/'
elif 'set3' in opt.dataroot:
    testDataRoot = './dataset/set3Test/'
elif 'set4' in opt.dataroot:
    testDataRoot = './dataset/set4Test/'
elif 'set5' in opt.dataroot:
    testDataRoot = './dataset/set5Test/'
else:
    sys.exit('Test data not found')
print(testDataRoot)
testDataset = folderPersonalizedForAcgan.ImageFolder(root=testDataRoot,
                                   transform=transforms.Compose([
                                   transforms.Resize((opt.imageSize,opt.imageSize)), ##since image original sizes are not square, take (size,size)
                                   transforms.ToTensor(),
                                   transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
...
testCorrect = 0
testDataTotalLength = len(testDataloader)
testDataLabels =[]
testDataPredictionScore = []
...
for i, data in enumerate(testDataloader, 0):
    ...
    testDataLabels = np.append(testDataLabels,label.detach().numpy())
    testDataPredictionScore = np.append(testDataPredictionScore,c_output[0,1].cpu().detach().numpy())
aucScore = roc_auc_score(testDataLabels,testDataPredictionScore)
print(testDataLabels)
print(testDataPredictionScore)
print('Test data: correct / total: %d / %d' % (testCorrect,testDataTotalLength)) 
print('AUC Score: %.4f'%(aucScore))
with open(opt.outf+'testResult.npy', 'wb') as f:
    np.save(f, testDataLabels)
    np.save(f, testDataPredictionScore)
